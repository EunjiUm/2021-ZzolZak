import os

import django

os.environ.setdefault("DJANGO_SETTINGS_MODULE", 'config.settings')
django.setup()

from index.models import *

FashionScore_musinsa_trench_coat = FashionScore_musinsa_trench_coat.objects.values()
FashionScore_seoulstore_trench_coat = FashionScore_seoulstore_trench_coat.objects.values()
FashionScore_ssf_trench_coat = FashionScore_ssf_trench_coat.objects.values()
FashionScore_musinsa_coat = FashionScore_musinsa_coat.objects.values()
FashionScore_seoulstore_coat = FashionScore_seoulstore_coat.objects.values()
FashionScore_ssf_coat = FashionScore_ssf_coat.objects.values()
FashionScore_musinsa_padded_jacket = FashionScore_musinsa_padded_jacket.objects.values()
FashionScore_seoulstore_padded_jacket = FashionScore_seoulstore_padded_jacket.objects.values()
FashionScore_ssf_padded_jacket = FashionScore_ssf_padded_jacket.objects.values()
FashionScore_musinsa_military = FashionScore_musinsa_military.objects.values()
FashionScore_seoulstore_military = FashionScore_seoulstore_military.objects.values()
FashionScore_ssf_military = FashionScore_ssf_military.objects.values()
FashionScore_musinsa_blazer = FashionScore_musinsa_blazer.objects.values()
FashionScore_seoulstore_blazer = FashionScore_seoulstore_blazer.objects.values()
FashionScore_ssf_blazer = FashionScore_ssf_blazer.objects.values()
FashionScore_musinsa_leather_jacket = FashionScore_musinsa_leather_jacket.objects.values()
FashionScore_seoulstore_leather_jacket = FashionScore_seoulstore_leather_jacket.objects.values()
FashionScore_ssf_leather_jacket = FashionScore_ssf_leather_jacket.objects.values()
FashionScore_musinsa_fur_jacket = FashionScore_musinsa_fur_jacket.objects.values()
FashionScore_seoulstore_fur_jacket = FashionScore_seoulstore_fur_jacket.objects.values()
FashionScore_ssf_fur_jacket = FashionScore_ssf_fur_jacket.objects.values()
FashionScore_musinsa_short_sleeve_jacket = FashionScore_musinsa_short_sleeve_jacket.objects.values()
FashionScore_seoulstore_short_sleeve_jacket = FashionScore_seoulstore_short_sleeve_jacket.objects.values()
FashionScore_ssf_short_sleeve_jacket = FashionScore_ssf_short_sleeve_jacket.objects.values()
FashionScore_musinsa_long_sleeve_jacket = FashionScore_musinsa_long_sleeve_jacket.objects.values()
FashionScore_seoulstore_long_sleeve_jacket = FashionScore_seoulstore_long_sleeve_jacket.objects.values()
FashionScore_ssf_long_sleeve_jacket = FashionScore_ssf_long_sleeve_jacket.objects.values()
FashionScore_musinsa_shirt = FashionScore_musinsa_shirt.objects.values()
FashionScore_seoulstore_shirt = FashionScore_seoulstore_shirt.objects.values()
FashionScore_ssf_shirt = FashionScore_ssf_shirt.objects.values()
FashionScore_musinsa_blouse = FashionScore_musinsa_blouse.objects.values()
FashionScore_seoulstore_blouse = FashionScore_seoulstore_blouse.objects.values()
FashionScore_ssf_blouse = FashionScore_ssf_blouse.objects.values()
FashionScore_musinsa_neat = FashionScore_musinsa_neat.objects.values()
FashionScore_seoulstore_neat = FashionScore_seoulstore_neat.objects.values()
FashionScore_ssf_neat = FashionScore_ssf_neat.objects.values()
FashionScore_musinsa_hoodie = FashionScore_musinsa_hoodie.objects.values()
FashionScore_seoulstore_hoodie = FashionScore_seoulstore_hoodie.objects.values()
FashionScore_ssf_hoodie = FashionScore_ssf_hoodie.objects.values()
FashionScore_musinsa_sweat_shirt = FashionScore_musinsa_sweat_shirt.objects.values()
FashionScore_seoulstore_sweat_shirt = FashionScore_seoulstore_sweat_shirt.objects.values()
FashionScore_ssf_sweat_shirt = FashionScore_ssf_sweat_shirt.objects.values()
FashionScore_musinsa_denim_pants = FashionScore_musinsa_denim_pants.objects.values()
FashionScore_seoulstore_denim_pants = FashionScore_seoulstore_denim_pants.objects.values()
FashionScore_ssf_denim_pants = FashionScore_ssf_denim_pants.objects.values()
FashionScore_musinsa_mini_skirt = FashionScore_musinsa_mini_skirt.objects.values()
FashionScore_seoulstore_mini_skirt = FashionScore_seoulstore_mini_skirt.objects.values()
FashionScore_ssf_mini_skirt = FashionScore_ssf_mini_skirt.objects.values()
FashionScore_musinsa_skirt = FashionScore_musinsa_skirt.objects.values()
FashionScore_seoulstore_skirt = FashionScore_seoulstore_skirt.objects.values()
FashionScore_ssf_skirt = FashionScore_ssf_skirt.objects.values()
FashionScore_musinsa_slacks = FashionScore_musinsa_slacks.objects.values()
FashionScore_seoulstore_slacks = FashionScore_seoulstore_slacks.objects.values()
FashionScore_ssf_slacks = FashionScore_ssf_slacks.objects.values()
FashionScore_musinsa_short_pants = FashionScore_musinsa_short_pants.objects.values()
FashionScore_seoulstore_short_pants = FashionScore_seoulstore_short_pants.objects.values()
FashionScore_ssf_short_pants = FashionScore_ssf_short_pants.objects.values()
FashionScore_musinsa_sports_wear = FashionScore_musinsa_sports_wear.objects.values()
FashionScore_seoulstore_sports_wear = FashionScore_seoulstore_sports_wear.objects.values()
FashionScore_ssf_sports_wear = FashionScore_ssf_sports_wear.objects.values()
FashionScore_musinsa_leggings = FashionScore_musinsa_leggings.objects.values()
FashionScore_seoulstore_leggings = FashionScore_seoulstore_leggings.objects.values()
FashionScore_ssf_leggings = FashionScore_ssf_leggings.objects.values()
FashionScore_musinsa_sports_shoes = FashionScore_musinsa_sports_shoes.objects.values()
FashionScore_seoulstore_sports_shoes = FashionScore_seoulstore_sports_shoes.objects.values()
FashionScore_ssf_sports_shoes = FashionScore_ssf_sports_shoes.objects.values()
FashionScore_musinsa_sandal = FashionScore_musinsa_sandal.objects.values()
FashionScore_seoulstore_sandal = FashionScore_seoulstore_sandal.objects.values()
FashionScore_ssf_sandal = FashionScore_ssf_sandal.objects.values()
FashionScore_musinsa_heel = FashionScore_musinsa_heel.objects.values()
FashionScore_seoulstore_heel = FashionScore_seoulstore_heel.objects.values()
FashionScore_ssf_heel = FashionScore_ssf_heel.objects.values()
FashionScore_musinsa_loafers = FashionScore_musinsa_loafers.objects.values()
FashionScore_seoulstore_loafers = FashionScore_seoulstore_loafers.objects.values()
FashionScore_ssf_loafers = FashionScore_ssf_loafers.objects.values()
FashionScore_musinsa_walker = FashionScore_musinsa_walker.objects.values()
FashionScore_seoulstore_walker = FashionScore_seoulstore_walker.objects.values()
FashionScore_ssf_walker = FashionScore_ssf_walker.objects.values()
FashionScore_musinsa_dress = FashionScore_musinsa_dress.objects.values()
FashionScore_seoulstore_dress = FashionScore_seoulstore_dress.objects.values()
FashionScore_ssf_dress = FashionScore_ssf_dress.objects.values()
FashionScore_musinsa_back_pack = FashionScore_musinsa_back_pack.objects.values()
FashionScore_seoulstore_back_pack = FashionScore_seoulstore_back_pack.objects.values()
FashionScore_ssf_back_pack = FashionScore_ssf_back_pack.objects.values()
FashionScore_musinsa_tote_bag = FashionScore_musinsa_tote_bag.objects.values()
FashionScore_seoulstore_tote_bag = FashionScore_seoulstore_tote_bag.objects.values()
FashionScore_ssf_tote_bag = FashionScore_ssf_tote_bag.objects.values()
FashionScore_musinsa_clutch_bag = FashionScore_musinsa_clutch_bag.objects.values()
FashionScore_seoulstore_clutch_bag = FashionScore_seoulstore_clutch_bag.objects.values()
FashionScore_ssf_clutch_bag = FashionScore_ssf_clutch_bag.objects.values()
FashionScore_musinsa_shoulder_bag = FashionScore_musinsa_shoulder_bag.objects.values()
FashionScore_seoulstore_shoulder_bag = FashionScore_seoulstore_shoulder_bag.objects.values()
FashionScore_ssf_shoulder_bag = FashionScore_ssf_shoulder_bag.objects.values()
FashionScore_musinsa_eco_bag = FashionScore_musinsa_eco_bag.objects.values()
FashionScore_seoulstore_eco_bag = FashionScore_seoulstore_eco_bag.objects.values()
FashionScore_ssf_eco_bag = FashionScore_ssf_eco_bag.objects.values()

reviewDate = [0] * 12
period = 0
year = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'June', 'July', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']


def save_FashionScore(product):
    if product == "trench_coat":
        for i in range(len(year)):
            reviewDate[i] = FashionScore_musinsa_trench_coat[period][year[i]] + \
                            FashionScore_seoulstore_trench_coat[period][year[i]] + \
                            FashionScore_ssf_trench_coat[period][year[i]]

        copied_list = sorted(reviewDate, reverse=True)
        month_1 = reviewDate.index(copied_list[0]) + 1
        month_2 = reviewDate.index(copied_list[1]) + 1
        month_3 = reviewDate.index(copied_list[2]) + 1
        FashionScore_trench_coat(
            Jan=reviewDate[0],
            Feb=reviewDate[1],
            Mar=reviewDate[2],
            Apr=reviewDate[3],
            May=reviewDate[4],
            June=reviewDate[5],
            July=reviewDate[6],
            Aug=reviewDate[7],
            Sep=reviewDate[8],
            Oct=reviewDate[9],
            Nov=reviewDate[10],
            Dec=reviewDate[11],
            month_1=month_1,
            month_2=month_2,
            month_3=month_3,
        ).save()
    elif product == "coat":
        for i in range(len(year)):
            reviewDate[i] = FashionScore_musinsa_coat[period][year[i]] + FashionScore_seoulstore_coat[period][year[i]] + \
                            FashionScore_ssf_coat[period][year[i]]
        copied_list = sorted(reviewDate, reverse=True)
        month_1 = reviewDate.index(copied_list[0]) + 1
        month_2 = reviewDate.index(copied_list[1]) + 1
        month_3 = reviewDate.index(copied_list[2]) + 1
        FashionScore_coat(
            Jan=reviewDate[0],
            Feb=reviewDate[1],
            Mar=reviewDate[2],
            Apr=reviewDate[3],
            May=reviewDate[4],
            June=reviewDate[5],
            July=reviewDate[6],
            Aug=reviewDate[7],
            Sep=reviewDate[8],
            Oct=reviewDate[9],
            Nov=reviewDate[10],
            Dec=reviewDate[11],
            month_1=month_1,
            month_2=month_2,
            month_3=month_3,
        ).save()
    elif product == "padded_jacket":
        for i in range(len(year)):
            reviewDate[i] = FashionScore_musinsa_padded_jacket[period][year[i]] + FashionScore_seoulstore_padded_jacket[period][
                year[i]] + FashionScore_ssf_padded_jacket[period][year[i]]
        copied_list = sorted(reviewDate, reverse=True)
        month_1 = reviewDate.index(copied_list[0]) + 1
        month_2 = reviewDate.index(copied_list[1]) + 1
        month_3 = reviewDate.index(copied_list[2]) + 1
        FashionScore_padded_jacket(
            Jan=reviewDate[0],
            Feb=reviewDate[1],
            Mar=reviewDate[2],
            Apr=reviewDate[3],
            May=reviewDate[4],
            June=reviewDate[5],
            July=reviewDate[6],
            Aug=reviewDate[7],
            Sep=reviewDate[8],
            Oct=reviewDate[9],
            Nov=reviewDate[10],
            Dec=reviewDate[11],
            month_1=month_1,
            month_2=month_2,
            month_3=month_3,
        ).save()
    elif product == "military":
        for i in range(len(year)):
            reviewDate[i] = FashionScore_musinsa_military[period][year[i]] + FashionScore_seoulstore_military[period][year[i]] + \
                            FashionScore_ssf_military[period][year[i]]
        copied_list = sorted(reviewDate, reverse=True)
        month_1 = reviewDate.index(copied_list[0]) + 1
        month_2 = reviewDate.index(copied_list[1]) + 1
        month_3 = reviewDate.index(copied_list[2]) + 1
        FashionScore_military(
            Jan=reviewDate[0],
            Feb=reviewDate[1],
            Mar=reviewDate[2],
            Apr=reviewDate[3],
            May=reviewDate[4],
            June=reviewDate[5],
            July=reviewDate[6],
            Aug=reviewDate[7],
            Sep=reviewDate[8],
            Oct=reviewDate[9],
            Nov=reviewDate[10],
            Dec=reviewDate[11],
            month_1=month_1,
            month_2=month_2,
            month_3=month_3,
        ).save()
    elif product == "blazer":
        for i in range(len(year)):
            reviewDate[i] = FashionScore_musinsa_blazer[period][year[i]] + FashionScore_seoulstore_blazer[period][year[i]] + \
                            FashionScore_ssf_blazer[period][year[i]]
        copied_list = sorted(reviewDate, reverse=True)
        month_1 = reviewDate.index(copied_list[0]) + 1
        month_2 = reviewDate.index(copied_list[1]) + 1
        month_3 = reviewDate.index(copied_list[2]) + 1
        FashionScore_blazer(
            Jan=reviewDate[0],
            Feb=reviewDate[1],
            Mar=reviewDate[2],
            Apr=reviewDate[3],
            May=reviewDate[4],
            June=reviewDate[5],
            July=reviewDate[6],
            Aug=reviewDate[7],
            Sep=reviewDate[8],
            Oct=reviewDate[9],
            Nov=reviewDate[10],
            Dec=reviewDate[11],
            month_1=month_1,
            month_2=month_2,
            month_3=month_3,
        ).save()
    elif product == "leather_jacket":
        for i in range(len(year)):
            reviewDate[i] = FashionScore_musinsa_leather_jacket[period][year[i]] + FashionScore_seoulstore_leather_jacket[period][
                year[i]] + FashionScore_ssf_leather_jacket[period][year[i]]
        copied_list = sorted(reviewDate, reverse=True)
        month_1 = reviewDate.index(copied_list[0]) + 1
        month_2 = reviewDate.index(copied_list[1]) + 1
        month_3 = reviewDate.index(copied_list[2]) + 1
        FashionScore_leather_jacket(
            Jan=reviewDate[0],
            Feb=reviewDate[1],
            Mar=reviewDate[2],
            Apr=reviewDate[3],
            May=reviewDate[4],
            June=reviewDate[5],
            July=reviewDate[6],
            Aug=reviewDate[7],
            Sep=reviewDate[8],
            Oct=reviewDate[9],
            Nov=reviewDate[10],
            Dec=reviewDate[11],
            month_1=month_1,
            month_2=month_2,
            month_3=month_3,
        ).save()
    elif product == "fur_jacket":
        for i in range(len(year)):
            reviewDate[i] = FashionScore_musinsa_fur_jacket[period][year[i]] + FashionScore_seoulstore_fur_jacket[period][
                year[i]] + FashionScore_ssf_fur_jacket[period][year[i]]
        copied_list = sorted(reviewDate, reverse=True)
        month_1 = reviewDate.index(copied_list[0]) + 1
        month_2 = reviewDate.index(copied_list[1]) + 1
        month_3 = reviewDate.index(copied_list[2]) + 1
        FashionScore_fur_jacket(
            Jan=reviewDate[0],
            Feb=reviewDate[1],
            Mar=reviewDate[2],
            Apr=reviewDate[3],
            May=reviewDate[4],
            June=reviewDate[5],
            July=reviewDate[6],
            Aug=reviewDate[7],
            Sep=reviewDate[8],
            Oct=reviewDate[9],
            Nov=reviewDate[10],
            Dec=reviewDate[11],
            month_1=month_1,
            month_2=month_2,
            month_3=month_3,
        ).save()
    elif product == "short_sleeve_jacket":
        for i in range(len(year)):
            reviewDate[i] = FashionScore_musinsa_short_sleeve_jacket[period][year[i]] + \
                            FashionScore_seoulstore_short_sleeve_jacket[period][year[i]] + \
                            FashionScore_ssf_short_sleeve_jacket[period][year[i]]
        copied_list = sorted(reviewDate, reverse=True)
        month_1 = reviewDate.index(copied_list[0]) + 1
        month_2 = reviewDate.index(copied_list[1]) + 1
        month_3 = reviewDate.index(copied_list[2]) + 1
        FashionScore_short_sleeve_jacket(
            Jan=reviewDate[0],
            Feb=reviewDate[1],
            Mar=reviewDate[2],
            Apr=reviewDate[3],
            May=reviewDate[4],
            June=reviewDate[5],
            July=reviewDate[6],
            Aug=reviewDate[7],
            Sep=reviewDate[8],
            Oct=reviewDate[9],
            Nov=reviewDate[10],
            Dec=reviewDate[11],
            month_1=month_1,
            month_2=month_2,
            month_3=month_3,
        ).save()
    elif product == "long_sleeve_jacket":
        for i in range(len(year)):
            reviewDate[i] = FashionScore_musinsa_long_sleeve_jacket[period][year[i]] + \
                            FashionScore_seoulstore_long_sleeve_jacket[period][year[i]] + \
                            FashionScore_ssf_long_sleeve_jacket[period][year[i]]
        copied_list = sorted(reviewDate, reverse=True)
        month_1 = reviewDate.index(copied_list[0]) + 1
        month_2 = reviewDate.index(copied_list[1]) + 1
        month_3 = reviewDate.index(copied_list[2]) + 1
        FashionScore_long_sleeve_jacket(
            Jan=reviewDate[0],
            Feb=reviewDate[1],
            Mar=reviewDate[2],
            Apr=reviewDate[3],
            May=reviewDate[4],
            June=reviewDate[5],
            July=reviewDate[6],
            Aug=reviewDate[7],
            Sep=reviewDate[8],
            Oct=reviewDate[9],
            Nov=reviewDate[10],
            Dec=reviewDate[11],
            month_1=month_1,
            month_2=month_2,
            month_3=month_3,
        ).save()
    elif product == "shirt":
        for i in range(len(year)):
            reviewDate[i] = FashionScore_musinsa_shirt[period][year[i]] + FashionScore_seoulstore_shirt[period][year[i]] + \
                            FashionScore_ssf_shirt[period][year[i]]
        copied_list = sorted(reviewDate, reverse=True)
        month_1 = reviewDate.index(copied_list[0]) + 1
        month_2 = reviewDate.index(copied_list[1]) + 1
        month_3 = reviewDate.index(copied_list[2]) + 1
        FashionScore_shirt(
            Jan=reviewDate[0],
            Feb=reviewDate[1],
            Mar=reviewDate[2],
            Apr=reviewDate[3],
            May=reviewDate[4],
            June=reviewDate[5],
            July=reviewDate[6],
            Aug=reviewDate[7],
            Sep=reviewDate[8],
            Oct=reviewDate[9],
            Nov=reviewDate[10],
            Dec=reviewDate[11],
            month_1=month_1,
            month_2=month_2,
            month_3=month_3,
        ).save()
    elif product == "blouse":
        for i in range(len(year)):
            reviewDate[i] = FashionScore_musinsa_blouse[period][year[i]] + FashionScore_seoulstore_blouse[period][year[i]] + \
                            FashionScore_ssf_blouse[period][year[i]]
        copied_list = sorted(reviewDate, reverse=True)
        month_1 = reviewDate.index(copied_list[0]) + 1
        month_2 = reviewDate.index(copied_list[1]) + 1
        month_3 = reviewDate.index(copied_list[2]) + 1
        FashionScore_blouse(
            Jan=reviewDate[0],
            Feb=reviewDate[1],
            Mar=reviewDate[2],
            Apr=reviewDate[3],
            May=reviewDate[4],
            June=reviewDate[5],
            July=reviewDate[6],
            Aug=reviewDate[7],
            Sep=reviewDate[8],
            Oct=reviewDate[9],
            Nov=reviewDate[10],
            Dec=reviewDate[11],
            month_1=month_1,
            month_2=month_2,
            month_3=month_3,
        ).save()
    elif product == "neat":
        for i in range(len(year)):
            reviewDate[i] = FashionScore_musinsa_neat[period][year[i]] + FashionScore_seoulstore_neat[period][year[i]] + \
                            FashionScore_ssf_neat[period][year[i]]
        copied_list = sorted(reviewDate, reverse=True)
        month_1 = reviewDate.index(copied_list[0]) + 1
        month_2 = reviewDate.index(copied_list[1]) + 1
        month_3 = reviewDate.index(copied_list[2]) + 1
        FashionScore_neat(
            Jan=reviewDate[0],
            Feb=reviewDate[1],
            Mar=reviewDate[2],
            Apr=reviewDate[3],
            May=reviewDate[4],
            June=reviewDate[5],
            July=reviewDate[6],
            Aug=reviewDate[7],
            Sep=reviewDate[8],
            Oct=reviewDate[9],
            Nov=reviewDate[10],
            Dec=reviewDate[11],
            month_1=month_1,
            month_2=month_2,
            month_3=month_3,
        ).save()
    elif product == "hoodie":
        for i in range(len(year)):
            reviewDate[i] = FashionScore_musinsa_hoodie[period][year[i]] + FashionScore_seoulstore_hoodie[period][year[i]] + \
                            FashionScore_ssf_hoodie[period][year[i]]
        copied_list = sorted(reviewDate, reverse=True)
        month_1 = reviewDate.index(copied_list[0]) + 1
        month_2 = reviewDate.index(copied_list[1]) + 1
        month_3 = reviewDate.index(copied_list[2]) + 1
        FashionScore_hoodie(
            Jan=reviewDate[0],
            Feb=reviewDate[1],
            Mar=reviewDate[2],
            Apr=reviewDate[3],
            May=reviewDate[4],
            June=reviewDate[5],
            July=reviewDate[6],
            Aug=reviewDate[7],
            Sep=reviewDate[8],
            Oct=reviewDate[9],
            Nov=reviewDate[10],
            Dec=reviewDate[11],
            month_1=month_1,
            month_2=month_2,
            month_3=month_3,
        ).save()
    elif product == "sweat_shirt":
        for i in range(len(year)):
            reviewDate[i] = FashionScore_musinsa_sweat_shirt[period][year[i]] + FashionScore_seoulstore_sweat_shirt[period][
                year[i]] + FashionScore_ssf_sweat_shirt[period][year[i]]
        copied_list = sorted(reviewDate, reverse=True)
        month_1 = reviewDate.index(copied_list[0]) + 1
        month_2 = reviewDate.index(copied_list[1]) + 1
        month_3 = reviewDate.index(copied_list[2]) + 1
        FashionScore_sweat_shirt(
            Jan=reviewDate[0],
            Feb=reviewDate[1],
            Mar=reviewDate[2],
            Apr=reviewDate[3],
            May=reviewDate[4],
            June=reviewDate[5],
            July=reviewDate[6],
            Aug=reviewDate[7],
            Sep=reviewDate[8],
            Oct=reviewDate[9],
            Nov=reviewDate[10],
            Dec=reviewDate[11],
            month_1=month_1,
            month_2=month_2,
            month_3=month_3,
        ).save()
    elif product == "denim_pants":
        for i in range(len(year)):
            reviewDate[i] = FashionScore_musinsa_denim_pants[period][year[i]] + FashionScore_seoulstore_denim_pants[period][
                year[i]] + FashionScore_ssf_denim_pants[period][year[i]]
        copied_list = sorted(reviewDate, reverse=True)
        month_1 = reviewDate.index(copied_list[0]) + 1
        month_2 = reviewDate.index(copied_list[1]) + 1
        month_3 = reviewDate.index(copied_list[2]) + 1
        FashionScore_denim_pants(
            Jan=reviewDate[0],
            Feb=reviewDate[1],
            Mar=reviewDate[2],
            Apr=reviewDate[3],
            May=reviewDate[4],
            June=reviewDate[5],
            July=reviewDate[6],
            Aug=reviewDate[7],
            Sep=reviewDate[8],
            Oct=reviewDate[9],
            Nov=reviewDate[10],
            Dec=reviewDate[11],
            month_1=month_1,
            month_2=month_2,
            month_3=month_3,
        ).save()
    elif product == "mini_skirt":
        for i in range(len(year)):
            reviewDate[i] = FashionScore_musinsa_mini_skirt[period][year[i]] + FashionScore_seoulstore_mini_skirt[period][
                year[i]] + FashionScore_ssf_mini_skirt[period][year[i]]
        copied_list = sorted(reviewDate, reverse=True)
        month_1 = reviewDate.index(copied_list[0]) + 1
        month_2 = reviewDate.index(copied_list[1]) + 1
        month_3 = reviewDate.index(copied_list[2]) + 1
        FashionScore_mini_skirt(
            Jan=reviewDate[0],
            Feb=reviewDate[1],
            Mar=reviewDate[2],
            Apr=reviewDate[3],
            May=reviewDate[4],
            June=reviewDate[5],
            July=reviewDate[6],
            Aug=reviewDate[7],
            Sep=reviewDate[8],
            Oct=reviewDate[9],
            Nov=reviewDate[10],
            Dec=reviewDate[11],
            month_1=month_1,
            month_2=month_2,
            month_3=month_3,
        ).save()
    elif product == "skirt":
        for i in range(len(year)):
            reviewDate[i] = FashionScore_musinsa_skirt[period][year[i]] + FashionScore_seoulstore_skirt[period][year[i]] + \
                            FashionScore_ssf_skirt[period][year[i]]
        copied_list = sorted(reviewDate, reverse=True)
        month_1 = reviewDate.index(copied_list[0]) + 1
        month_2 = reviewDate.index(copied_list[1]) + 1
        month_3 = reviewDate.index(copied_list[2]) + 1
        FashionScore_skirt(
            Jan=reviewDate[0],
            Feb=reviewDate[1],
            Mar=reviewDate[2],
            Apr=reviewDate[3],
            May=reviewDate[4],
            June=reviewDate[5],
            July=reviewDate[6],
            Aug=reviewDate[7],
            Sep=reviewDate[8],
            Oct=reviewDate[9],
            Nov=reviewDate[10],
            Dec=reviewDate[11],
            month_1=month_1,
            month_2=month_2,
            month_3=month_3,
        ).save()
    elif product == "slacks":
        for i in range(len(year)):
            reviewDate[i] = FashionScore_musinsa_slacks[period][year[i]] + FashionScore_seoulstore_slacks[period][year[i]] + \
                            FashionScore_ssf_slacks[period][year[i]]
        copied_list = sorted(reviewDate, reverse=True)
        month_1 = reviewDate.index(copied_list[0]) + 1
        month_2 = reviewDate.index(copied_list[1]) + 1
        month_3 = reviewDate.index(copied_list[2]) + 1
        FashionScore_slacks(
            Jan=reviewDate[0],
            Feb=reviewDate[1],
            Mar=reviewDate[2],
            Apr=reviewDate[3],
            May=reviewDate[4],
            June=reviewDate[5],
            July=reviewDate[6],
            Aug=reviewDate[7],
            Sep=reviewDate[8],
            Oct=reviewDate[9],
            Nov=reviewDate[10],
            Dec=reviewDate[11],
            month_1=month_1,
            month_2=month_2,
            month_3=month_3,
        ).save()
    elif product == "short_pants":
        for i in range(len(year)):
            reviewDate[i] = FashionScore_musinsa_short_pants[period][year[i]] + FashionScore_seoulstore_short_pants[period][
                year[i]] + FashionScore_ssf_short_pants[period][year[i]]
        copied_list = sorted(reviewDate, reverse=True)
        month_1 = reviewDate.index(copied_list[0]) + 1
        month_2 = reviewDate.index(copied_list[1]) + 1
        month_3 = reviewDate.index(copied_list[2]) + 1
        FashionScore_short_pants(
            Jan=reviewDate[0],
            Feb=reviewDate[1],
            Mar=reviewDate[2],
            Apr=reviewDate[3],
            May=reviewDate[4],
            June=reviewDate[5],
            July=reviewDate[6],
            Aug=reviewDate[7],
            Sep=reviewDate[8],
            Oct=reviewDate[9],
            Nov=reviewDate[10],
            Dec=reviewDate[11],
            month_1=month_1,
            month_2=month_2,
            month_3=month_3,
        ).save()
    elif product == "sports_wear":
        for i in range(len(year)):
            reviewDate[i] = FashionScore_musinsa_sports_wear[period][year[i]] + FashionScore_seoulstore_sports_wear[period][
                year[i]] + FashionScore_ssf_sports_wear[period][year[i]]
        copied_list = sorted(reviewDate, reverse=True)
        month_1 = reviewDate.index(copied_list[0]) + 1
        month_2 = reviewDate.index(copied_list[1]) + 1
        month_3 = reviewDate.index(copied_list[2]) + 1
        FashionScore_sports_wear(
            Jan=reviewDate[0],
            Feb=reviewDate[1],
            Mar=reviewDate[2],
            Apr=reviewDate[3],
            May=reviewDate[4],
            June=reviewDate[5],
            July=reviewDate[6],
            Aug=reviewDate[7],
            Sep=reviewDate[8],
            Oct=reviewDate[9],
            Nov=reviewDate[10],
            Dec=reviewDate[11],
            month_1=month_1,
            month_2=month_2,
            month_3=month_3,
        ).save()
    elif product == "leggings":
        for i in range(len(year)):
            reviewDate[i] = FashionScore_musinsa_leggings[period][year[i]] + FashionScore_seoulstore_leggings[period][year[i]] + \
                            FashionScore_ssf_leggings[period][year[i]]
        copied_list = sorted(reviewDate, reverse=True)
        month_1 = reviewDate.index(copied_list[0]) + 1
        month_2 = reviewDate.index(copied_list[1]) + 1
        month_3 = reviewDate.index(copied_list[2]) + 1
        FashionScore_leggings(
            Jan=reviewDate[0],
            Feb=reviewDate[1],
            Mar=reviewDate[2],
            Apr=reviewDate[3],
            May=reviewDate[4],
            June=reviewDate[5],
            July=reviewDate[6],
            Aug=reviewDate[7],
            Sep=reviewDate[8],
            Oct=reviewDate[9],
            Nov=reviewDate[10],
            Dec=reviewDate[11],
            month_1=month_1,
            month_2=month_2,
            month_3=month_3,
        ).save()
    elif product == "sports_shoes":
        for i in range(len(year)):
            reviewDate[i] = FashionScore_musinsa_sports_shoes[period][year[i]] + FashionScore_seoulstore_sports_shoes[period][
                year[i]] + FashionScore_ssf_sports_shoes[period][year[i]]
        copied_list = sorted(reviewDate, reverse=True)
        month_1 = reviewDate.index(copied_list[0]) + 1
        month_2 = reviewDate.index(copied_list[1]) + 1
        month_3 = reviewDate.index(copied_list[2]) + 1
        FashionScore_sports_shoes(
            Jan=reviewDate[0],
            Feb=reviewDate[1],
            Mar=reviewDate[2],
            Apr=reviewDate[3],
            May=reviewDate[4],
            June=reviewDate[5],
            July=reviewDate[6],
            Aug=reviewDate[7],
            Sep=reviewDate[8],
            Oct=reviewDate[9],
            Nov=reviewDate[10],
            Dec=reviewDate[11],
            month_1=month_1,
            month_2=month_2,
            month_3=month_3,
        ).save()
    elif product == "sandal":
        for i in range(len(year)):
            reviewDate[i] = FashionScore_musinsa_sandal[period][year[i]] + FashionScore_seoulstore_sandal[period][year[i]] + \
                            FashionScore_ssf_sandal[period][year[i]]
        copied_list = sorted(reviewDate, reverse=True)
        month_1 = reviewDate.index(copied_list[0]) + 1
        month_2 = reviewDate.index(copied_list[1]) + 1
        month_3 = reviewDate.index(copied_list[2]) + 1
        FashionScore_sandal(
            Jan=reviewDate[0],
            Feb=reviewDate[1],
            Mar=reviewDate[2],
            Apr=reviewDate[3],
            May=reviewDate[4],
            June=reviewDate[5],
            July=reviewDate[6],
            Aug=reviewDate[7],
            Sep=reviewDate[8],
            Oct=reviewDate[9],
            Nov=reviewDate[10],
            Dec=reviewDate[11],
            month_1=month_1,
            month_2=month_2,
            month_3=month_3,
        ).save()
    elif product == "heel":
        for i in range(len(year)):
            reviewDate[i] = FashionScore_musinsa_heel[period][year[i]] + FashionScore_seoulstore_heel[period][year[i]] + \
                            FashionScore_ssf_heel[period][year[i]]
        copied_list = sorted(reviewDate, reverse=True)
        month_1 = reviewDate.index(copied_list[0]) + 1
        month_2 = reviewDate.index(copied_list[1]) + 1
        month_3 = reviewDate.index(copied_list[2]) + 1
        FashionScore_heel(
            Jan=reviewDate[0],
            Feb=reviewDate[1],
            Mar=reviewDate[2],
            Apr=reviewDate[3],
            May=reviewDate[4],
            June=reviewDate[5],
            July=reviewDate[6],
            Aug=reviewDate[7],
            Sep=reviewDate[8],
            Oct=reviewDate[9],
            Nov=reviewDate[10],
            Dec=reviewDate[11],
            month_1=month_1,
            month_2=month_2,
            month_3=month_3,
        ).save()
    elif product == "loafers":
        for i in range(len(year)):
            reviewDate[i] = FashionScore_musinsa_loafers[period][year[i]] + FashionScore_seoulstore_loafers[period][year[i]] + \
                            FashionScore_ssf_loafers[period][year[i]]
        copied_list = sorted(reviewDate, reverse=True)
        month_1 = reviewDate.index(copied_list[0]) + 1
        month_2 = reviewDate.index(copied_list[1]) + 1
        month_3 = reviewDate.index(copied_list[2]) + 1
        FashionScore_loafers(
            Jan=reviewDate[0],
            Feb=reviewDate[1],
            Mar=reviewDate[2],
            Apr=reviewDate[3],
            May=reviewDate[4],
            June=reviewDate[5],
            July=reviewDate[6],
            Aug=reviewDate[7],
            Sep=reviewDate[8],
            Oct=reviewDate[9],
            Nov=reviewDate[10],
            Dec=reviewDate[11],
            month_1=month_1,
            month_2=month_2,
            month_3=month_3,
        ).save()
    elif product == "walker":
        for i in range(len(year)):
            reviewDate[i] = FashionScore_musinsa_walker[period][year[i]] + FashionScore_seoulstore_walker[period][year[i]] + \
                            FashionScore_ssf_walker[period][year[i]]
        copied_list = sorted(reviewDate, reverse=True)
        month_1 = reviewDate.index(copied_list[0]) + 1
        month_2 = reviewDate.index(copied_list[1]) + 1
        month_3 = reviewDate.index(copied_list[2]) + 1
        FashionScore_walker(
            Jan=reviewDate[0],
            Feb=reviewDate[1],
            Mar=reviewDate[2],
            Apr=reviewDate[3],
            May=reviewDate[4],
            June=reviewDate[5],
            July=reviewDate[6],
            Aug=reviewDate[7],
            Sep=reviewDate[8],
            Oct=reviewDate[9],
            Nov=reviewDate[10],
            Dec=reviewDate[11],
            month_1=month_1,
            month_2=month_2,
            month_3=month_3,
        ).save()
    elif product == "dress":
        for i in range(len(year)):
            reviewDate[i] = FashionScore_musinsa_dress[period][year[i]] + FashionScore_seoulstore_dress[period][year[i]] + \
                            FashionScore_ssf_dress[period][year[i]]
        copied_list = sorted(reviewDate, reverse=True)
        month_1 = reviewDate.index(copied_list[0]) + 1
        month_2 = reviewDate.index(copied_list[1]) + 1
        month_3 = reviewDate.index(copied_list[2]) + 1
        FashionScore_dress(
            Jan=reviewDate[0],
            Feb=reviewDate[1],
            Mar=reviewDate[2],
            Apr=reviewDate[3],
            May=reviewDate[4],
            June=reviewDate[5],
            July=reviewDate[6],
            Aug=reviewDate[7],
            Sep=reviewDate[8],
            Oct=reviewDate[9],
            Nov=reviewDate[10],
            Dec=reviewDate[11],
            month_1=month_1,
            month_2=month_2,
            month_3=month_3,
        ).save()
    elif product == "back_pack":
        for i in range(len(year)):
            reviewDate[i] = FashionScore_musinsa_back_pack[period][year[i]] + FashionScore_seoulstore_back_pack[period][year[i]] + \
                            FashionScore_ssf_back_pack[period][year[i]]
        copied_list = sorted(reviewDate, reverse=True)
        month_1 = reviewDate.index(copied_list[0]) + 1
        month_2 = reviewDate.index(copied_list[1]) + 1
        month_3 = reviewDate.index(copied_list[2]) + 1
        FashionScore_back_pack(
            Jan=reviewDate[0],
            Feb=reviewDate[1],
            Mar=reviewDate[2],
            Apr=reviewDate[3],
            May=reviewDate[4],
            June=reviewDate[5],
            July=reviewDate[6],
            Aug=reviewDate[7],
            Sep=reviewDate[8],
            Oct=reviewDate[9],
            Nov=reviewDate[10],
            Dec=reviewDate[11],
            month_1=month_1,
            month_2=month_2,
            month_3=month_3,
        ).save()
    elif product == "tote_bag":
        for i in range(len(year)):
            reviewDate[i] = FashionScore_musinsa_tote_bag[period][year[i]] + FashionScore_seoulstore_tote_bag[period][year[i]] + \
                            FashionScore_ssf_tote_bag[period][year[i]]
        copied_list = sorted(reviewDate, reverse=True)
        month_1 = reviewDate.index(copied_list[0]) + 1
        month_2 = reviewDate.index(copied_list[1]) + 1
        month_3 = reviewDate.index(copied_list[2]) + 1
        FashionScore_tote_bag(
            Jan=reviewDate[0],
            Feb=reviewDate[1],
            Mar=reviewDate[2],
            Apr=reviewDate[3],
            May=reviewDate[4],
            June=reviewDate[5],
            July=reviewDate[6],
            Aug=reviewDate[7],
            Sep=reviewDate[8],
            Oct=reviewDate[9],
            Nov=reviewDate[10],
            Dec=reviewDate[11],
            month_1=month_1,
            month_2=month_2,
            month_3=month_3,
        ).save()
    elif product == "clutch_bag":
        for i in range(len(year)):
            reviewDate[i] = FashionScore_musinsa_clutch_bag[period][year[i]] + FashionScore_seoulstore_clutch_bag[period][
                year[i]] + FashionScore_ssf_clutch_bag[period][year[i]]
        copied_list = sorted(reviewDate, reverse=True)
        month_1 = reviewDate.index(copied_list[0]) + 1
        month_2 = reviewDate.index(copied_list[1]) + 1
        month_3 = reviewDate.index(copied_list[2]) + 1
        FashionScore_clutch_bag(
            Jan=reviewDate[0],
            Feb=reviewDate[1],
            Mar=reviewDate[2],
            Apr=reviewDate[3],
            May=reviewDate[4],
            June=reviewDate[5],
            July=reviewDate[6],
            Aug=reviewDate[7],
            Sep=reviewDate[8],
            Oct=reviewDate[9],
            Nov=reviewDate[10],
            Dec=reviewDate[11],
            month_1=month_1,
            month_2=month_2,
            month_3=month_3,
        ).save()
    elif product == "shoulder_bag":
        for i in range(len(year)):
            reviewDate[i] = FashionScore_musinsa_shoulder_bag[period][year[i]] + FashionScore_seoulstore_shoulder_bag[period][
                year[i]] + FashionScore_ssf_shoulder_bag[period][year[i]]
        copied_list = sorted(reviewDate, reverse=True)
        month_1 = reviewDate.index(copied_list[0]) + 1
        month_2 = reviewDate.index(copied_list[1]) + 1
        month_3 = reviewDate.index(copied_list[2]) + 1
        FashionScore_shoulder_bag(
            Jan=reviewDate[0],
            Feb=reviewDate[1],
            Mar=reviewDate[2],
            Apr=reviewDate[3],
            May=reviewDate[4],
            June=reviewDate[5],
            July=reviewDate[6],
            Aug=reviewDate[7],
            Sep=reviewDate[8],
            Oct=reviewDate[9],
            Nov=reviewDate[10],
            Dec=reviewDate[11],
            month_1=month_1,
            month_2=month_2,
            month_3=month_3,
        ).save()
    elif product == "eco_bag":
        for i in range(len(year)):
            reviewDate[i] = FashionScore_musinsa_eco_bag[period][year[i]] + FashionScore_seoulstore_eco_bag[period][year[i]] + \
                            FashionScore_ssf_eco_bag[period][year[i]]
        copied_list = sorted(reviewDate, reverse=True)
        month_1 = reviewDate.index(copied_list[0]) + 1
        month_2 = reviewDate.index(copied_list[1]) + 1
        month_3 = reviewDate.index(copied_list[2]) + 1
        FashionScore_eco_bag(
            Jan=reviewDate[0],
            Feb=reviewDate[1],
            Mar=reviewDate[2],
            Apr=reviewDate[3],
            May=reviewDate[4],
            June=reviewDate[5],
            July=reviewDate[6],
            Aug=reviewDate[7],
            Sep=reviewDate[8],
            Oct=reviewDate[9],
            Nov=reviewDate[10],
            Dec=reviewDate[11],
            month_1=month_1,
            month_2=month_2,
            month_3=month_3,
        ).save()


def save_Ranking(product):
    if product == "trench_coat":
        Ranking(
            ranking_name="트렌치코트",
            ranking_img=product + ".jpeg",
            ranking_score=FashionScore_musinsa_trench_coat[period]['ranking_score'] +
                          FashionScore_seoulstore_trench_coat[period][
                              'ranking_score'] + FashionScore_ssf_trench_coat[period]['ranking_score']
        ).save()
    elif product == "coat":
        Ranking(
            ranking_name="코트",
            ranking_img=product + ".jpeg",
            ranking_score=FashionScore_musinsa_coat[period]['ranking_score'] +
                          FashionScore_seoulstore_coat[period][
                              'ranking_score'] + FashionScore_ssf_coat[period]['ranking_score']
        ).save()
    elif product == "padded_jacket":
        Ranking(
            ranking_name="패딩",
            ranking_img=product + ".jpeg",
            ranking_score=FashionScore_musinsa_padded_jacket[period]['ranking_score'] +
                          FashionScore_seoulstore_padded_jacket[period][
                              'ranking_score'] + FashionScore_ssf_padded_jacket[period]['ranking_score']
        ).save()
    elif product == "military":
        Ranking(
            ranking_name="밀리터리",
            ranking_img=product + ".jpeg",
            ranking_score=FashionScore_musinsa_military[period]['ranking_score'] +
                          FashionScore_seoulstore_military[period][
                              'ranking_score'] + FashionScore_ssf_military[period]['ranking_score']
        ).save()
    elif product == "blazer":
        Ranking(
            ranking_name="블레이저",
            ranking_img=product + ".jpeg",
            ranking_score=FashionScore_musinsa_blazer[period]['ranking_score'] +
                          FashionScore_seoulstore_blazer[period][
                              'ranking_score'] + FashionScore_ssf_blazer[period]['ranking_score']
        ).save()
    elif product == "leather_jacket":
        Ranking(
            ranking_name="레더재킷",
            ranking_img=product + ".jpeg",
            ranking_score=FashionScore_musinsa_leather_jacket[period]['ranking_score'] +
                          FashionScore_seoulstore_leather_jacket[period][
                              'ranking_score'] + FashionScore_ssf_leather_jacket[period]['ranking_score']
        ).save()
    elif product == "fur_jacket":
        Ranking(
            ranking_name="퍼",
            ranking_img=product + ".jpeg",
            ranking_score=FashionScore_musinsa_fur_jacket[period]['ranking_score'] +
                          FashionScore_seoulstore_fur_jacket[period][
                              'ranking_score'] + FashionScore_ssf_fur_jacket[period]['ranking_score']
        ).save()
    elif product == "short_sleeve_jacket":
        Ranking(
            ranking_name="반팔 티셔츠",
            ranking_img=product + ".jpeg",
            ranking_score=FashionScore_musinsa_short_sleeve_jacket[period]['ranking_score'] +
                          FashionScore_seoulstore_short_sleeve_jacket[period][
                              'ranking_score'] + FashionScore_ssf_short_sleeve_jacket[period]['ranking_score']
        ).save()
    elif product == "long_sleeve_jacket":
        Ranking(
            ranking_name="긴팔 티셔츠",
            ranking_img=product + ".jpeg",
            ranking_score=FashionScore_musinsa_long_sleeve_jacket[period]['ranking_score'] +
                          FashionScore_seoulstore_long_sleeve_jacket[period][
                              'ranking_score'] + FashionScore_ssf_long_sleeve_jacket[period]['ranking_score']
        ).save()
    elif product == "shirt":
        Ranking(
            ranking_name="셔츠",
            ranking_img=product + ".jpeg",
            ranking_score=FashionScore_musinsa_shirt[period]['ranking_score'] +
                          FashionScore_seoulstore_shirt[period][
                              'ranking_score'] + FashionScore_ssf_shirt[period]['ranking_score']
        ).save()
    elif product == "blouse":
        Ranking(
            ranking_name="블라우스",
            ranking_img=product + ".jpeg",
            ranking_score=FashionScore_musinsa_blouse[period]['ranking_score'] +
                          FashionScore_seoulstore_blouse[period][
                              'ranking_score'] + FashionScore_ssf_blouse[period]['ranking_score']
        ).save()
    elif product == "neat":
        Ranking(
            ranking_name="니트",
            ranking_img=product + ".jpeg",
            ranking_score=FashionScore_musinsa_neat[period]['ranking_score'] +
                          FashionScore_seoulstore_neat[period][
                              'ranking_score'] + FashionScore_ssf_neat[period]['ranking_score']
        ).save()
    elif product == "hoodie":
        Ranking(
            ranking_name="후드",
            ranking_img=product + ".jpeg",
            ranking_score=FashionScore_musinsa_hoodie[period]['ranking_score'] +
                          FashionScore_seoulstore_hoodie[period][
                              'ranking_score'] + FashionScore_ssf_hoodie[period]['ranking_score']
        ).save()
    elif product == "sweat_shirt":
        Ranking(
            ranking_name="맨투맨",
            ranking_img=product + ".jpeg",
            ranking_score=FashionScore_musinsa_sweat_shirt[period]['ranking_score'] +
                          FashionScore_seoulstore_sweat_shirt[period][
                              'ranking_score'] + FashionScore_ssf_sweat_shirt[period]['ranking_score']
        ).save()
    elif product == "denim_pants":
        Ranking(
            ranking_name="데님팬츠",
            ranking_img=product + ".jpeg",
            ranking_score=FashionScore_musinsa_denim_pants[period]['ranking_score'] +
                          FashionScore_seoulstore_denim_pants[period][
                              'ranking_score'] + FashionScore_ssf_denim_pants[period]['ranking_score']
        ).save()
    elif product == "mini_skirt":
        Ranking(
            ranking_name="미니스커트",
            ranking_img=product + ".jpeg",
            ranking_score=FashionScore_musinsa_mini_skirt[period]['ranking_score'] +
                          FashionScore_seoulstore_mini_skirt[period][
                              'ranking_score'] + FashionScore_ssf_mini_skirt[period]['ranking_score']
        ).save()
    elif product == "skirt":
        Ranking(
            ranking_name="스커트",
            ranking_img=product + ".jpeg",
            ranking_score=FashionScore_musinsa_skirt[period]['ranking_score'] +
                          FashionScore_seoulstore_skirt[period][
                              'ranking_score'] + FashionScore_ssf_skirt[period]['ranking_score']
        ).save()
    elif product == "slacks":
        Ranking(
            ranking_name="슬랙스",
            ranking_img=product + ".jpeg",
            ranking_score=FashionScore_musinsa_slacks[period]['ranking_score'] +
                          FashionScore_seoulstore_slacks[period][
                              'ranking_score'] + FashionScore_ssf_slacks[period]['ranking_score']
        ).save()
    elif product == "short_pants":
        Ranking(
            ranking_name="숏팬츠",
            ranking_img=product + ".jpeg",
            ranking_score=FashionScore_musinsa_short_pants[period]['ranking_score'] +
                          FashionScore_seoulstore_short_pants[period][
                              'ranking_score'] + FashionScore_ssf_short_pants[period]['ranking_score']
        ).save()
    elif product == "sports_wear":
        Ranking(
            ranking_name="트레이닝",
            ranking_img=product + ".jpeg",
            ranking_score=FashionScore_musinsa_sports_wear[period]['ranking_score'] +
                          FashionScore_seoulstore_sports_wear[period][
                              'ranking_score'] + FashionScore_ssf_sports_wear[period]['ranking_score']
        ).save()
    elif product == "leggings":
        Ranking(
            ranking_name="레깅스",
            ranking_img=product + ".jpeg",
            ranking_score=FashionScore_musinsa_leggings[period]['ranking_score'] +
                          FashionScore_seoulstore_leggings[period][
                              'ranking_score'] + FashionScore_ssf_leggings[period]['ranking_score']
        ).save()
    elif product == "sports_shoes":
        Ranking(
            ranking_name="운동화",
            ranking_img=product + ".jpeg",
            ranking_score=FashionScore_musinsa_sports_shoes[period]['ranking_score'] +
                          FashionScore_seoulstore_sports_shoes[period][
                              'ranking_score'] + FashionScore_ssf_sports_shoes[period]['ranking_score']
        ).save()
    elif product == "sandal":
        Ranking(
            ranking_name="샌들",
            ranking_img=product + ".jpeg",
            ranking_score=FashionScore_musinsa_sandal[period]['ranking_score'] +
                          FashionScore_seoulstore_sandal[period][
                              'ranking_score'] + FashionScore_ssf_sandal[period]['ranking_score']
        ).save()
    elif product == "heel":
        Ranking(
            ranking_name="힐",
            ranking_img=product + ".jpeg",
            ranking_score=FashionScore_musinsa_heel[period]['ranking_score'] +
                          FashionScore_seoulstore_heel[period][
                              'ranking_score'] + FashionScore_ssf_heel[period]['ranking_score']
        ).save()
    elif product == "loafers":
        Ranking(
            ranking_name="로퍼",
            ranking_img=product + ".jpeg",
            ranking_score=FashionScore_musinsa_loafers[period]['ranking_score'] +
                          FashionScore_seoulstore_loafers[period][
                              'ranking_score'] + FashionScore_ssf_loafers[period]['ranking_score']
        ).save()
    elif product == "walker":
        Ranking(
            ranking_name="워커",
            ranking_img=product + ".jpeg",
            ranking_score=FashionScore_musinsa_walker[period]['ranking_score'] +
                          FashionScore_seoulstore_walker[period][
                              'ranking_score'] + FashionScore_ssf_walker[period]['ranking_score']
        ).save()
    elif product == "dress":
        Ranking(
            ranking_name="원피스",
            ranking_img=product + ".jpeg",
            ranking_score=FashionScore_musinsa_dress[period]['ranking_score'] +
                          FashionScore_seoulstore_dress[period][
                              'ranking_score'] + FashionScore_ssf_dress[period]['ranking_score']
        ).save()
    elif product == "back_pack":
        Ranking(
            ranking_name="백팩",
            ranking_img=product + ".jpeg",
            ranking_score=FashionScore_musinsa_back_pack[period]['ranking_score'] +
                          FashionScore_seoulstore_back_pack[period][
                              'ranking_score'] + FashionScore_ssf_back_pack[period]['ranking_score']
        ).save()
    elif product == "tote_bag":
        Ranking(
            ranking_name="토트백",
            ranking_img=product + ".jpeg",
            ranking_score=FashionScore_musinsa_tote_bag[period]['ranking_score'] +
                          FashionScore_seoulstore_tote_bag[period][
                              'ranking_score'] + FashionScore_ssf_tote_bag[period]['ranking_score']
        ).save()
    elif product == "clutch_bag":
        Ranking(
            ranking_name="클러치",
            ranking_img=product + ".jpeg",
            ranking_score=FashionScore_musinsa_clutch_bag[period]['ranking_score'] +
                          FashionScore_seoulstore_clutch_bag[period][
                              'ranking_score'] + FashionScore_ssf_clutch_bag[period]['ranking_score']
        ).save()
    elif product == "shoulder_bag":
        Ranking(
            ranking_name="숄더백",
            ranking_img=product + ".jpeg",
            ranking_score=FashionScore_musinsa_shoulder_bag[period]['ranking_score'] +
                          FashionScore_seoulstore_shoulder_bag[period][
                              'ranking_score'] + FashionScore_ssf_shoulder_bag[period]['ranking_score']
        ).save()
    elif product == "eco_bag":
        Ranking(
            ranking_name="에코백",
            ranking_img=product + ".jpeg",
            ranking_score=FashionScore_musinsa_eco_bag[period]['ranking_score'] +
                          FashionScore_seoulstore_eco_bag[period][
                              'ranking_score'] + FashionScore_ssf_eco_bag[period]['ranking_score']
        ).save()

def txt_Integration():
    company_list = ['musinsa', 'ssf', 'seoulstore']
    product_list = ['trench_coat',
                    'coat',
                    'padded_jacket',
                    'military',
                    'blazer',
                    'leather_jacket',
                    'fur_jacket',
                    'short_sleeve_jacket',
                    'long_sleeve_jacket',
                    'shirt',
                    'blouse',
                    'neat',
                    'hoodie',
                    'sweat_shirt',
                    'denim_pants',
                    'mini_skirt',
                    'skirt',
                    'slacks',
                    'short_pants',
                    'sports_wear',
                    'leggings',
                    'sports_shoes',
                    'sandal',
                    'heel',
                    'loafers',
                    'walker',
                    'dress',
                    'back_pack',
                    'tote_bag',
                    'clutch_bag',
                    'shoulder_bag',
                    'eco_bag']
    for product in product_list:
        file = open("data/" + product + ".txt", 'w', encoding='utf-8')
        for company in company_list:
            filepath = "data/" + company + "_" + product + ".txt"
            f = open(filepath, 'r', encoding='utf-8')
            for line in f.readlines():
                file.write(line)
            f.close()
        file.close()


#txt_Integration()
product_list = ['trench_coat',
                    'coat',
                    'padded_jacket',
                    'military',
                    'blazer',
                    'leather_jacket',
                    'fur_jacket',
                    'short_sleeve_jacket',
                    'long_sleeve_jacket',
                    'shirt',
                    'blouse',
                    'neat',
                    'hoodie',
                    'sweat_shirt',
                    'denim_pants',
                    'mini_skirt',
                    'skirt',
                    'slacks',
                    'short_pants',
                    'sports_wear',
                    'leggings',
                    'sports_shoes',
                    'sandal',
                    'heel',
                    'loafers',
                    'walker',
                    'dress',
                    'back_pack',
                    'tote_bag',
                    'clutch_bag',
                    'shoulder_bag',
                    'eco_bag']
for product in product_list:
    save_FashionScore(product)
    save_Ranking(product)